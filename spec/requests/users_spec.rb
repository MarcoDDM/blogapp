...
RSpec.describe User, type: :system do
  before do
    ...
    jane
    brian
  end  context 'index page test group 1' do
    before { visit root_path }    it 'shows the username of each user' do
      expect(page).to have_content(tom.name)
      expect(page).to have_content(nancy.name)
      expect(page).to have_content(jane.name)
      expect(page).to have_content(brian.name)
    end    it 'shows the profile picture of each user' do
      profile_pictures = all('.user-image.border')      expect(profile_pictures[0]).to be_visible
  ...
  end  context 'index page test group 2' do
    before { visit root_path }    it 'shows the number of posts each user has written' do
      user_tiles = all('.card')      user_tiles.each do |user_tile|
        expect(user_tile).to have_content(/\d+ Post/)
      end
    end    it 'redirects to a user\'s show page on click' do
      click_link tom.name      toms_show_page_url = user_path(id: tom.id)      expect(page).to have_current_path(toms_show_page_url)
    end
  end  context 'show page test group 1' do
    subject { brian }    before do
      first_post
      second_post
      third_post
      visit user_path(subject)
    end    it 'shows the user profile picture' do
      profile_picture = find('.user-image')      expect(profile_picture).to be_visible
      expect(profile_picture['src']).to eq subject.photo
    end    it 'shows the user\'s name' do
      expect(page).to have_content(subject.name)
    end    it 'shows the number of posts the user has written' do
      expect(page).to have_content("#{subject.posts_counter} Post#{subject.posts_counter == 1 ? '' : 's'}")
    end    it 'shows the user bio' do
      expect(page).to have_content("Brian Tally\n3 Posts\nAdd a post\n@@ -19,19 +19,19 @@\nWelcome to my blog\nThis is my first post\nComments: 0 Likes: 0\nWelcome to my second post\nThis is my second post\nComments: 0 Likes: 0\nWelcome to my third post\nThis is my third post\nComments: 0 Likes: 0\nSee all posts")
    end    it 'shows the user\'s first three posts' do
    ...
    end    it 'shows a button that shows all the user\'s posts when clicked' do
      expect(page).to have_link('See all posts')
    end
  end  context 'show page test group 2' do
    subject { brian }    before do
      first_post
      second_post
      third_post
      visit user_path(subject)
    end    it 'redirects to the show page of a post when it is clicked' do
      first_post_url = user_post_path(user_id: subject.id, id: subject.posts.first.id)      click_link subject.posts.first.title      expect(page).to have_current_path(first_post_url)
      expect(page).to have_content(subject.posts.first.title)
      expect(page).to have_content(subject.posts.first.text)
    end    it 'redirects to the post\'s index page when the \'See all posts\' link is clicked' do
      all_posts_url = user_posts_path(user_id: subject.id)      click_link 'See all posts'
